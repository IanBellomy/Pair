...
class exports.Pair extends Framer.EventEmitter
	...
	constructor: (@anchor, @floater) ->
		...
		@dragStartHandler = (event,layer) =>
			...
			@_hoveredNode = document.elementFromPoint(event.clientX, event.clientY) #.style['background-color']			
			@floater.visible = true
			@emit "dragStart"

		@dragHandler = (event) =>			
			@floater.visible = false
			nodeUnderneath = document.elementFromPoint(event.clientX, event.clientY) #.style['background-color']			
			@floater.visible = true
	...
	# the cursor should really be aptured somehow.
